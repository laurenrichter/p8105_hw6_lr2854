...
The values of $\hat r^2$ and $log(\beta_0*\beta_1)$ were pulled and calculated by unnesting the `broom::glance()` and `broom::tidy()` functions, respectively. These were plotted with the resulting densities demonstrating that the sample approximates a normal distribution (mean $\approx$ median). 
...
where ${S_{\bar X}} = \frac{s}{{\sqrt N }}$ (standard error)
...
get_ci = function (x, pct_conf = 95) {
  ...
  results_tibble = tibble(res_info = results_)
